import React, { Component } from "react";
import { variables } from "../utils/Variables.js";

import "../styles/autobusat.css";


export class Autobusat extends Component {
  constructor(props) {
    super(props);

    this.state = {
      oraret: [],
      autobusat: [],
      modalTitle: "",
      AutobusatId: 0,
      AutobusatName: "",
      Orari: "",
      OraNisjes: "",
      OraMberritjes: "",
    };
  }

  refreshList() {
    fetch(variables.API_URL + "orari")
      .then((response) => response.json())
      .then((data) => {
        this.setState({ oraret: data });
      });

    fetch(variables.API_URL + "autobusat")
      .then((response) => response.json())
      .then((data) => {
        this.setState({ autobusat: data });
      });
  }

  componentDidMount() {
    this.refreshList();
  }

  changeAutobusatName = (e) => {
    this.setState({ AutobusatName: e.target.value });
  };
  changeOrari = (e) => {
    this.setState({ Orari: e.target.value });
  };
  changeOraNisjes = (e) => {
    this.setState({ OraNisjes: e.target.value });
  };
  changeOraMberritjes = (e) => {
    this.setState({ OraMberritjes: e.target.value });
  };

  addClick() {
    this.setState({
      modalTitle: "Shto Autobusin",
      AutobusatId: 0,
      AutobusatName: "",
      Orari: "",
      OraNisjes: "",
      OraMberritjes: "",
    });
  }

  editClick(auto) {
    this.setState({
      modalTitle: "Edit Autobusin",
      AutobusatId: auto.AutobusatId,
      AutobusatName: auto.AutobusatName,
      Orari: auto.Orari,
      OraNisjes: auto.OraNisjes,
      OraMberritjes: auto.OraMberritjes,
    });
  }

  createClick() {
    fetch(variables.API_URL + "autobusat", {
      method: "POST",
      headers: {
        Accept: "application/json",
        "Content-Type": "application/json",
      },
      body: JSON.stringify({
        AutobusatName: this.state.AutobusatName,
        Orari: this.state.Orari,
        OraNisjes: this.state.OraNisjes,
        OraMberritjes: this.state.OraMberritjes,
      }),
    })
      .then((res) => res.json())
      .then(
        (result) => {
          alert(result);
          this.refreshList();
        },
        (error) => {
          alert("Failed");
        }
      );
  }

  updateClick() {
    fetch(variables.API_URL + "autobusat", {
      method: "PUT",
      headers: {
        Accept: "application/json",
        "Content-Type": "application/json",
      },
      body: JSON.stringify({
        AutobusatId:this.state.AutobusatId,
        AutobusatName: this.state.AutobusatName,
        Orari: this.state.Orari,
        OraNisjes: this.state.OraNisjes,
        OraMberritjes: this.state.OraMberritjes,
      }),
    })
      .then((res) => res.json())
      .then(
        (result) => {
          alert(result);
          this.refreshList();
        },
        (error) => {
          alert("Failed");
        }
      );
  }

  deleteClick(id) {
    if (window.confirm("A jeni i sigurt?")) {
      fetch(variables.API_URL + "autobusat/" + id, {
        method: "DELETE",
        headers: {
          Accept: "application/json",
          "Content-Type": "application/json",
        },
      })
        .then((res) => res.json())
        .then(
          (result) => {
            alert(result);
            this.refreshList();
          },
          (error) => {
            alert("Failed");
          }
        );
    }
  }

  render() {
    const {
      autobusat,
      oraret,
      modalTitle,
      AutobusatId,
      AutobusatName,
      Orari,
      OraNisjes,
      OraMberritjes,
    } = this.state;
    return (
    <div>
    <div className="back">
    <button className="bback"><a href="Navbar">Back</a></button>
    </div>
        <div class="AutobusatApp">
          <button
            type="button"
            className="btn btn-primary m-2 float-end"
            data-bs-toggle="modal"
            data-bs-target="#exampleModal"
            onClick={() => this.addClick()}
          >
            Shto Autobusat
          </button>
          <table className="table table-striped">
            <thead>
              <tr>
                <th>AutobusatId</th>
                <th>AutobusatName</th>
                <th>Orari</th>
                <th>OraNisjes</th>
                <th>OraMberritjes</th>
                <th>Options</th>
              </tr>
            </thead>
            <tbody>
              {autobusat.map((auto) => (
                <tr key={auto.AutobusatId}>
                  <td>{auto.AutobusatId}</td>
                  <td>{auto.AutobusatName}</td>
                  <td>{auto.Orari}</td>
                  <td>{auto.OraNisjes}</td>
                  <td>{auto.OraMberritjes}</td>
                  <td>
                    <button
                      type="button"
                      className="btn btn-light mr-1"
                      data-bs-toggle="modal"
                      data-bs-target="#exampleModal"
                      onClick={() => this.editClick(auto)}
                    >
                      <svg
                        xmlns="http://www.w3.org/2000/svg"
                        width="16"
                        height="16"
                        fill="currentColor"
                        className="bi bi-pencil-square"
                        viewBox="0 0 16 16"
                      >
                        <path d="M15.502 1.94a.5.5 0 0 1 0 .706L14.459 3.69l-2-2L13.502.646a.5.5 0 0 1 .707 0l1.293 1.293zm-1.75 2.456-2-2L4.939 9.21a.5.5 0 0 0-.121.196l-.805 2.414a.25.25 0 0 0 .316.316l2.414-.805a.5.5 0 0 0 .196-.12l6.813-6.814z" />
                        <path
                          fillRule="evenodd"
                          d="M1 13.5A1.5 1.5 0 0 0 2.5 15h11a1.5 1.5 0 0 0 1.5-1.5v-6a.5.5 0 0 0-1 0v6a.5.5 0 0 1-.5.5h-11a.5.5 0 0 1-.5-.5v-11a.5.5 0 0 1 .5-.5H9a.5.5 0 0 0 0-1H2.5A1.5 1.5 0 0 0 1 2.5v11z"
                        />
                      </svg>
                    </button>
                    <button
                      type="button"
                      className="btn btn-light mr-1"
                      onClick={() => this.deleteClick(auto.AutobusatId)}
                    >
                      <svg
                        xmlns="http://www.w3.org/2000/svg"
                        width="16"
                        height="16"
                        fill="currentColor"
                        className="bi bi-trash-fill"
                        viewBox="0 0 16 16"
                      >
                        <path d="M2.5 1a1 1 0 0 0-1 1v1a1 1 0 0 0 1 1H3v9a2 2 0 0 0 2 2h6a2 2 0 0 0 2-2V4h.5a1 1 0 0 0 1-1V2a1 1 0 0 0-1-1H10a1 1 0 0 0-1-1H7a1 1 0 0 0-1 1H2.5zm3 4a.5.5 0 0 1 .5.5v7a.5.5 0 0 1-1 0v-7a.5.5 0 0 1 .5-.5zM8 5a.5.5 0 0 1 .5.5v7a.5.5 0 0 1-1 0v-7A.5.5 0 0 1 8 5zm3 .5v7a.5.5 0 0 1-1 0v-7a.5.5 0 0 1 1 0z" />
                      </svg>
                    </button>
                  </td>
                </tr>
              ))}
            </tbody>
          </table>
          <div
            className="modal fade"
            id="exampleModal"
            tabIndex="-1"
            aria-hidden="true"
          >
            <div className="modal-dialog modal-lg modal-dialog-centered">
              <div className="modal-content">
                <div className="modal-header">
                  <h5 className="modal-title">{modalTitle}</h5>
                  <button
                    type="button"
                    className="btn-close"
                    data-bs-dismiss="modal"
                    aria-label="Close"
                  ></button>
                </div>

                <div className="modal-body">
                  <div className="d-flex flex-row bd-highlight mb-3">
                    <div className="p-2 w-50 bd-highlight">
                      <div className="input-group mb-3">
                        <span className="input-group-text">Autobusi Name</span>
                        <input
                          type="text"
                          className="form-control"
                          value={AutobusatName}
                          onChange={this.changeAutobusatName}
                        />
                      </div>
                      <div className="input-group mb-3">
                        <span className="input-group-text">Orari</span>
                        <select
                          className="form-select"
                          onChange={this.changeOrari}
                          value={Orari}
                        >
                          {oraret.map((ora) => (
                            <option key={ora.OrariId}>{ora.OrariName}</option>
                          ))}
                        </select>
                      </div>
                      <div className="input-group mb-3">
                        <span className="input-group-text">OraNisjes</span>
                        <input
                          type="time"
                          className="form-control"
                          value={OraNisjes}
                          onChange={this.changeOraNisjes}
                        />
                      </div>
                      <div className="input-group mb-3">
                        <span className="input-group-text">OraMberritjes</span>
                        <input
                          type="time"
                          className="form-control"
                          value={OraMberritjes}
                          onChange={this.changeOraMberritjes}
                        />
                      </div>
                  </div>
                  {AutobusatId === 0 ? (
                    <button
                      type="button"
                      className="btn btn-primary float-start"
                      onClick={() => this.createClick()}
                    >
                      Create
                    </button>
                  ) : null}

                  {AutobusatId !== 0 ? (
                    <button
                      type="button"
                      className="btn btn-primary float-start"
                      onClick={() => this.updateClick()}
                    >
                      Update
                    </button>
                  ) : null}
                </div>
              </div>
            </div>
          </div>
        </div>
      </div>
    </div>
    );
  }
}
